      ...
      <div className="flex gap-3 overflow-x-auto pb-2 lg:block lg:space-y-3 lg:overflow-visible lg:pb-0">
        {frameworkLayers.map((layer, index) => (
          <button
            ...
            className={`min-w-[260px] rounded-[1.5rem] border p-5 text-left transition duration-300 lg:w-full ${index === active ? "border-gold bg-petroleum text-white shadow-premium" : "border-petroleum/10 bg-white/80 text-petroleum hover:-translate-y-0.5 hover:border-gold/40 hover:bg-white dark:border-white/10 dark:bg-white/5 dark:text-sand"}`}
          >
            ...
            <div className="mt-3 font-display text-2xl font-semibold leading-tight sm:text-3xl">{layer.title}</div>
          </button>
        ))}
      </div>
      <div className="relative overflow-hidden rounded-[2rem] border border-petroleum/10 bg-white p-6 shadow-premium dark:border-white/10 dark:bg-white/5 sm:rounded-[2.5rem] lg:p-10">
        ...
        <div className="relative">
          ...
          <h3 className="mt-6 font-display text-4xl font-semibold leading-tight text-petroleum sm:text-5xl dark:text-sand">{item.title}</h3>
          <div className="mt-6 rounded-3xl border border-gold/20 bg-gold/10 p-5 dark:bg-gold/10">
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-2">
            <div className="rounded-3xl bg-petroleum p-6 text-sand dark:bg-sand dark:text-petroleum">
              <div className="text-xs font-bold uppercase tracking-[0.24em] text-gold dark:text-petroleum/65">O que ainda não está definido</div>
              <p className="mt-2 text-sm leading-7 text-sand/82 dark:text-petroleum/75">{item.notDefined}</p>
            </div>
